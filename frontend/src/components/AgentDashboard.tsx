...
const AgentDashboard: React.FC = () => {
    ...
    // --- THIS IS THE FIX ---
    // The key handler logic is now inside a focus-aware useEffect hook.
    useEffect(() => {
        const handleKeyDown = (e: KeyboardEvent) => {
            ...
            // --- CHANGED FROM e.altKey to e.ctrlKey ---
            else if (e.ctrlKey) { 
                e.preventDefault();
                getAISuggestion();
            } 
        ...
        const addListeners = () => { window.addEventListener('keydown', handleKeyDown); window.addEventListener('keyup', handleKeyUp); };
        const removeListeners = () => { window.removeEventListener('keydown', handleKeyDown); window.removeEventListener('keyup', handleKeyUp); };
    ...
    }, [currentQuery, isListening, takeNextQuery, getAISuggestion, startListening, stopListening]);
    ...
    return (
        <div className="flex flex-col h-screen bg-gradient-to-br from-[#2c2a4a] to-[#4f3b78] text-white">
            <header className="flex justify-between items-center p-4 bg-black/20 shrink-0">
                <div><h1 className="text-2xl font-bold">Agent Portal</h1><p className="text-sm text-gray-400">Welcome back, Agent {user?.name}</p></div>
                <button onClick={logout} className="px-5 py-2 bg-red-600 font-bold rounded-lg">Logout</button>
            </header>
            <main className="flex-1 p-6 overflow-y-auto space-y-6">
                <div className="flex gap-6">
                    <StatCard title="Pending Queries" count={pendingCount} icon="⏲️" /><StatCard title="In Progress" count={inProgressCount} icon="💬" /><StatCard title="Completed" count={completedCount} icon="✔️" />
                </div>
                <div className="bg-black/20 p-6 rounded-xl border border-white/10 text-center">
                    {!currentQuery ? (
                        <><h2 className="text-2xl font-bold">Ready for Next Query</h2><p className="text-gray-400 mt-1">There are {pendingCount} customer{pendingCount !== 1 ? 's' : ''} waiting.</p></>
                    ) : (
                        <div className="text-left space-y-4">
                            <h3 className="font-bold text-lg">Current Query from: <span className="text-teal-300">{currentQuery.userEmail}</span></h3><p className="p-3 bg-gray-800/50 rounded-lg">{currentQuery.text}</p>
                            <h3 className="font-bold text-lg">AI Suggested Response:</h3>
                            {/* --- UPDATE THE UI HINT --- */}
                            <p className="p-3 bg-gray-800/50 rounded-lg italic h-16">{currentQuery.aiSuggestion || "Press CTRL key to generate suggestion..."}</p>
                            <hr className="border-white/10" />
                            <div><h3 className="font-bold text-lg">Your Final Response:</h3><div className={`p-4 mt-2 text-center rounded-lg border-2 ${isListening ? 'border-red-500 animate-pulse' : 'border-dashed border-gray-600'}`}><p className="font-semibold">{isListening ? "Listening..." : "Hold SHIFT to record your response"}</p></div></div>
                        </div>
                    )}
                </div>
